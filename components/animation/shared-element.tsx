  ...
  return (
    <>
      <motion.div
        ...
        whileHover="hover"
        variants={imageGridItemVariants}
        transition={springTransition}
        style={{ cursor: "pointer" }}
      >
        <motion.img
...
interface SharedElementTransitionProps {
  from: { layoutId: string; content: ReactNode }
  to: { layoutId: string; content: ReactNode }
  isActive: boolean
  className?: string
}
...
export function SharedElementTransition({
  ...
  return (
    <AnimatePresence mode="popLayout">
      <motion.div
        key={isActive ? to.layoutId : from.layoutId}
        layoutId={isActive ? to.layoutId : from.layoutId}
        className={className}
        transition={springTransition}
      >
        {isActive ? to.content : from.content}
      </motion.div>
    </AnimatePresence>
  )
}
...
export function CardExpand({
  ...
  return (
    <motion.div
      ...
      style={{ cursor: "pointer" }}
    >
      <motion.div layoutId={`card-title-${title}`} className="font-semibold">
        {title}
      </motion.div>
      <motion.div
      ...
      {!isExpanded && <div className="pt-2">{previewContent}</div>}
    </motion.div>
  )
}

export { springTransition, imageGridItemVariants }