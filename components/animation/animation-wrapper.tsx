...
export function AnimationWrapper({
  ...
  const shouldReduceMotion = useReducedMotion()
  const { shouldReducedMotion } = useReducedMotionContext()

  const finalShouldReduce = shouldReduceMotion || shouldReducedMotion

  const variants = customVariants || getAnimationPreset(preset)

  const resolvedVariants: Variants = finalShouldReduce
    ? { visible: { opacity: 1 } }
    : variants

  let motionTransition = transition

  if (!finalShouldReduce && !transition) {
    motionTransition = {
      ...springTransition,
      delay,
...
export function FadeReveal({
  ...
  return (
    <motion.div
      ...
      whileInView="visible"
      variants={
        shouldReduceMotion ? { visible: { opacity: 1 } } : directionVariants[direction]
      }
